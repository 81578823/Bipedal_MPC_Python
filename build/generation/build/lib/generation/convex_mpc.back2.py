...
class ConvexMPC:
    ...
    def __init__(self, nodeHandle: Node,
        ...
        self.dim = hpipm_ocp_qp_dim(self.N)    # too slow
        self.dim.set('nx', nx, 0, self.N) # number of states
        self.dim.set('nu', nu, 0, self.N-1) # number of inputs
        # qp sol
        ...
        arg.set('pred_corr', 0)
        arg.set('split_step', 0)

        self.solver = hpipm_ocp_qp_solver(self.dim, arg)
    ...
    def optimize(self):
        ...
        # self.getDynamics(t_now, 0, mode_schedule)
        # self.getInequalityConstraints(0, N, mode_schedule)
        # self.getCosts(t_now, 0, N, mode_schedule)
            
        range_end_time = time.time()
        print('range {:e}'.format(range_end_time-range_time))
        ...
        # codegen
        # if codegen_data:
        #     self.dim.codegen('dense_qp_data.c', 'w')

        qp = hpipm_ocp_qp(self.dim)
        ...
        qp.set('A', self.ocp_[0]["A"], 0,N-1)
        qp.set('B', self.ocp_[0]["B"], 0,N-1)
        qp.set('b', self.ocp_[0]["b"], 0,N-1)
        qp.set('Q', self.ocp_[0]["Q"], 0,N-1)
        qp.set('S', self.ocp_[0]["S"], 0,N-1)
        qp.set('R', self.ocp_[0]["R"], 0,N-1)
        qp.set('q', self.ocp_[0]["q"], 0,N-1)
        qp.set('r', self.ocp_[0]["r"], 0,N-1)
        # qp.set('C', self.ocp_[0]["C"], 0,N-1)
        qp.set('D', self.ocp_[0]["D"], 0,N-1)
        qp.set('lg', self.ocp_[0]["lg"], 0,N-1)
        qp.set('ug', self.ocp_[0]["ug"], 0,N-1)
        qp.set('lg_mask', self.ocp_[0]["lg_mask"], 0,N-1)
        qp.set('ug_mask', self.ocp_[0]["ug_mask"], 0,N-1)
        qp.set('lx', x0, 0)
        # qp.set('ux', x0, 0)
        ...
        # codegen
        # if codegen_data:
        #     self.arg.codegen('ocp_qp_data.c', 'a')

        # set up solver
        # solver = hpipm_ocp_qp_solver(self.dim, self.arg)

        # solve_time = time.time()
        # solve qp
        self.solver.solve(qp, self.qp_sol)    # don't matter
        # solve_time_end = time.time()
        # print('solve {:e}'.format(solve_time_end-solve_time))

        for k in range(N):
            x = self.qp_sol.get('x', k)
            u = self.qp_sol.get('u', k)
        ...
        print('optimize {:e}'.format(end_time-start_time))
